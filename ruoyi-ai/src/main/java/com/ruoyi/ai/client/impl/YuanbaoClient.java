 ...
 */
@Component
public class YuanbaoClient implements AiClient
{
    private static final Logger log = LoggerFactory.getLogger(YuanbaoClient.class);

    private static final String PLATFORM_CODE = "yuanbao";
    private static final String DEFAULT_API_URL = "https://hunyuan.tencentcloudapi.com/v1/chat/completions";
    ...
    public AiChat chat(String apiKey, String apiUrl, String model, String message)
    {
        ...
        {
            log.error("元宝API调用失败: {}", e.getMessage(), e);
            chat.setAiResponse("调用元宝API失败: " + e.getMessage());
        }
    ...
    private AiChat parseResponse(AiChat chat, JSONObject response)
    {
        ...
        {
            log.error("解析元宝响应失败: {}", e.getMessage(), e);
